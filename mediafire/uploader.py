...
class MediaFireUploader(object):
    ...
    def upload(self, fd, name=None, folder_key=None, filedrop_key=None,
               path=None, action_on_duplicate=None,print_progress_meter=True):
        ...
        while retries > 0:
            try:
                ...
                upload_result = upload_func(upload_info, check_result,print_progress_meter=print_progress_meter)
    ...
    def _poll_upload(self, upload_key, action):
        ...
        while quick_key is None:
            ...
            try:
                ...
                logger.debug("poll(%s): status=%d, description=%s, "
                         " result=%d",
    ...
    def _upload_none(self, upload_info, check_result,print_progress_meter=False):
    ...
    def _upload_instant(self, upload_info, _=None,print_progress_meter=False):
    ...
    def _upload_simple(self, upload_info, _=None,print_progress_meter=False):
    ...
    def _upload_resumable_all(self, upload_info, bitmap,
                              number_of_units, unit_size,print_progress_meter=True):
        ...
        iterhere=range(number_of_units)
        if print_progress_meter: iterhere=tqdm.tqdm(range(number_of_units),desc='mf_upload')
        
        for unit_id in iterhere:
    ...
    def _upload_resumable(self, upload_info, check_result,print_progress_meter=True):
        ...
        while not all_units_ready and retries > 0:
            upload_key = self._upload_resumable_all(upload_info, bitmap,
                                                    number_of_units, unit_size,print_progress_meter=print_progress_meter)
